...
"use strict";
(function (w) {
  var version="1.2.1",ctx,svg='http://www.w3.org/2000/svg',gT={},
      gdoStroke=true,gdoFill=true,gStrokeCap='round',
      gfillSet=false,gstrokeSet=false,gStk=[],
      gFill="#fff",gStroke="#000",gWeight=1,looping=false,vertices=[],
  ...
  w.pixelDensity = function(val) {
    if(!ctx) console.log("'pixelDensity' unsupported for svg");
    else {
      w._pixelDensity = val;
  ...
  w.scale = function(x,y) {
    if(!ctx) console.log("'scale' unsupported for svg");
    else {
  ...
  w.clear = function() {
    if(!ctx) console.log("'clear' unsupported for svg");
  ...
  w.fill = function(r,g,b,a) {
    ...
    if(ctx) ctx.fillStyle = gFill;
  };

  w.noFill = function() {gdoFill = false;};
  ...
  w.stroke = function(r,g,b,a) {
    ...
    if(ctx) ctx.strokeStyle = gStroke;
  };

  w.strokeWeight = function(w) {
    gWeight = w;
    gdoStroke = true;
    if(ctx) ctx.lineWidth = gWeight;
  };

  w.strokeCap = function(val) {
    gStrokeCap = val;
    if(ctx) ctx.lineCap = val;
  };

  w.noStroke = function() {gdoStroke = false;};

  w.cursor = function(c) {canv.style.cursor = c;};
  ...
  w.shearX = function(r) {
    if(!ctx) console.log("'shearX' unsupported for svg");
  ...
  w.push = function() {
    if(!ctx) gStk.push({gTx:gT.x,gTy:gT.y,gTr:gT.r,
  ...
  w.pop = function() {
    ...
    if(!ctx) gT.x=o.gTx,gT.y=o.gTy,gT.r=o.gTr;
    else ctx.restore();
  };

  // TEXT
  w.textSize = function(s) {gtextSize = s;};
  w.textStyle = function(s) {gtextStyle = s;};
  w.textFont = function(s) {gtextFont = s;};
  ...
  w.endShape = function(mode) {
    ...
    if(!ctx) {
      var e=document.createElementNS(svg,'path'), path;
  ...
  w.loop = function() {
    if(looping)return;
    looping=true;
    requestAnimationFrame(w._update);
  };
  w.noLoop = function() {looping=false;};
  w._update = function() {
    if(ctx) ctx.setTransform(w._pixelDensity,0,0,w._pixelDensity,0,0);
    w.draw();
    w.frameCount++;
    if(looping)requestAnimationFrame(w._update);
  }
  if(typeof w === "function")w(w); // instantiate non-global mode
  if(w.setup) {w.setup();
    if(w.draw) {looping=true;requestAnimationFrame(w._update);}
  }
})(sketch);

}
//global sketch instance
//document.onreadystatechange=function(){new p5(window, "_");}
document.addEventListener("readystatechange", function(event) {
  if(event.target.readyState === "complete") new p5(window, "_");
});